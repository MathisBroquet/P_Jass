...
namespace P_Jass
{
    class Slide
    {
        ...
        public Slide(string title, string text1, string text2)
        {
        ...
        private void DisplaySingleText(string text1)
        {
            ...
            for (int i = 0; i < System.Console.WindowHeight - 7; i++)
            {
                ...
                {
                    if (count < wrd1.Length && tempcount + wrd1[count].Split('\n')[0].Length + 1 <= System.Console.WindowWidth - 2)
                    {
                        if (n)
                        {
                            break;
                        }
                        var test = wrd1[count].Split('\n', 2);
                        if (wrd1[count].Split('\n', 2).Length >= 1)
                        {
                            System.Console.Write(wrd1[count].Split('\n')[0] + " ");
                            tempcount += wrd1[count].Split('\n')[0].Length + 1;
                        }
                        else
                        {
                            System.Console.Write(wrd1[count] + " ");
